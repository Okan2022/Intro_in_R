...
# Chapter 2: Data Manipulation/Wrangling/Bridging/Munging
...
pacman::p_load("tidyverse", "haven", "psych", "skim", "WDI", "OECD") 
...
# the p_needed object.
p_needed <-
  c("tidyverse","haven", "psych","skim", "WDI", "OECD")
...
|File           |File Extension | Command (Package)                            |
|---------------|---------------|----------------------------------------------|
...
head(d3) #Lets have a look if everything worked out
...
head(d4) #Lets have a look if everything worked out 
...
+ Using statistics to artificially fill them out, this called multiple
imputation techniques, you will learn that in AQM. 

+ Just delete incomplete observations to have a dataset without missing values

We will do the second one, and I already showed how to recode useless values to NAs so this should be clear by now. In the following code. Remember, the `ifelse()` und `recode` explicitly need input to turn values into NAs, `case_when()` however, turns values, which are not explicitly stated into NAs.


```{r}


#For that we take our dataset d6 
...
### 1.) You are interested in discrimination and the perception of the judicative. More specifically, you want to know if people, who fell discriminated evaluate courts differently. Below you see a table with all variables you want to include in your analysis: 
