...
pop <- read_dta("party_means.dta")
parl <- read.csv("parlgov_election.csv")
```
...
```{r merge}
parl18 <- parl %>% filter(election_date <= 2019) %>% filter(election_date >= 2017)
...
neth <- comp %>% filter(comp$country_name == "Netherlands")
uk <- comp %>% filter(comp$country_name == "United Kingdom")
ger <- comp %>% filter(comp$country_name == "Germany")
